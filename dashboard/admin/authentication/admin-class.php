...
class ADMIN
{
        ...
        if($otp == $_SESSION['OTP']){
            ...
            <body>
                <div class='container'>
                    ...
                    <p>Welcome to ITELEC ACTIVITY</p>
            ...
            $this->send_email($email, $message, $subject, $this->smtp_email, $this->smtp_password);
            echo "<script>alert('Thank you'); window.location.href = '../../../verify-otp.php';</script>";
    ...
    public function addAdmin($csrf_token, $username, $email, $password)
    {
        ...
        $stmt = $this->runQuery('INSERT INTO user (username, email, password) VALUE (:username, :email, :password)');
        ...
        if($exec){
            echo "<script>alert('Admin Added Sucessfully.'); window.location.href = '../../..';</script>";
            exit;
        }else{
            ...
            if($userRow->rowCount() ==1){
                if($userRow['status' == "active"]){
                    if($userRow['password'] == md5($password)){
                        ...
                        echo "<script>alert('Password if incorrect'); window.location.href = '../../../';</script>";
                        exit;  
                    }
                }else{
    ...
    function send_email($email, $message, $subject, $smtp_email, $smtp_password){
        ...
        $mail->setFrom($smtp_email, "ITELEC ACTIVITY");
